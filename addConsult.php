...
// retrieve Patient details for populating drop down
$queryPat = "SELECT pat_id, pat_name FROM patient ORDER BY pat_id;";
$resultPat = mysql_query($queryPat);
if (!$resultPat) {
    die('Invalid Patient query: ' . mysql_error());
}
$optionPat = '';
 while($rowPat = mysql_fetch_assoc($resultPat))
{
  $optionPat .= '<option value = "'.$rowPat['pat_id'].'">'.$rowPat['pat_name'].'</option>';
}
// retrieve Test details for populating drop down
$queryT = "SELECT doc_id, doc_name FROM doctor ORDER BY doc_id;";
$resultT = mysql_query($queryT);
if (!$resultT) {
    die('Invalid Doctor query: ' . mysql_error());
}
$optionDoc = '';
 while($rowT = mysql_fetch_assoc($resultT))
{
  $optionDoc .= '<option value = "'.$rowT['doc_id'].'">'.$rowT['doc_name'].'</option>';
}
?>
	...
	<div class="services-breadcrumb">
		<div class="container">
			<ul>
				...
				<li>Consultation Appointment<i>></i></li>
				<li>Add Consultation Appointment</li>
			</ul>
		</div>
	</div>
<!-- //banner1 -->
<!-- icons -->
	<div class="banner-bottom" id="about">
		<div class="container">
			<p class="sub_t_agileits"><b><u>Add a new Consultation Appointment</u></b></p>

			<div class="book-appointment">
				<form action="addConsult_proc.php" method="post">
					<div class="left-agileits-w3layouts same">
						<div class="gaps">
							<p>Patient Name</p>
							<select id="pat_id" name="pat_id">
								<?php echo $optionPat; ?>
							</select>
						</div>
						<div class="gaps">	
						...
						<div class="gaps">
							<p>Consultation Date (yyyy-mm-dd)</p>
							<input type="text" name="c_date" placeholder="" required="" />
						</div>	
					</div>
					<div class="clearfix"></div>
					<input type="submit" value="Add Consultation Appointment">
				</form>
			</div>
		</div>
	</div>
